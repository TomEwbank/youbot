...
%            drawnow;
       ...
       if full_rotate == 0 %On attend qu'il tourne après chaque trajectoire puis on calcule les next points
           if new_traject == 1
                [points, map] = planNextMove(map, pose, d, cell_size, fill_point);
                points
                s = size(points);
                index = 1;
                new_traject = 0;
           ...
           if new_point == 1 %Détermine une nouvelle droite à suivre
               b_line = - (x_star - x);
               a_line = y_star - y;
               c_line = - (a_line * x) - (b_line * y);
               new_point = 0;
               x_init = x;
               y_init = y;
           end
           
           if map(floor((x_star+d)/cell_size)+1, floor((y_star+d)/cell_size)+1) == 1
               new_traject = 1;
           end
           ...
           %Distance entre le point actuel du youbot et la droite
           d_line =  ((a_line * x) + (y * b_line) + c_line)/(sqrt(a_line^2 + b_line^2));
           distance_before_objective = sqrt((x_star - x)^2+(y_star -y)^2);
           if distance_before_objective < 1.5
               precision = 3/180*pi;
           else
               precision = 6/180*pi;
           end
           if abs(angdiff(theta_star, theta)) > precision
               forwBackVel = 0;
               fsm = 'recalibrate';
           else
               rotVel = 0;
      ...
      elseif strcmp(fsm, 'recalibrate')
%           %On ajoute une vitesse perpendiculaire en fonction de la
%           %distance
%           forwBackVel = 0;
%           leftRightVel = 3*d_line; %A modifier pour vitesse
%            if (x_star - x_init)*(y - y_init) - (y_star - y_init)*(x - x_init) > 0
%                leftRightVel = - leftRightVel; 
%            end
            
           rotVel = 10*angdiff(theta_star, theta);
           if abs(angdiff(theta_star, theta)) < 1/180*pi
               rotVel = 0;
               fsm = 'drive';
               display('yo');
...
%             abs(youbotPos(2) - y_star)
%             abs(youbotPos(1) - x_star)
...
%            end
        ...
        %    if youbotPos(1)+3.167 < .001,
           if abs(youbotPos(2) - y_star) < .1 && abs(youbotPos(1) - x_star ) < .1
