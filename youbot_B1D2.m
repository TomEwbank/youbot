...
%                 trajmap(corner_index(corner_num,1)-2,corner_index(corner_num,2)-2) = 45;
%             end
        end
        figure(1)
        trajmap(a,b) = 3;
    ...
    if strcmp(fsm, 'starting')
        ...
        % form the robot starting position
...
%                 table2_ind = floor((table2+d)/cell_size)+1;
                ...
                if corner_dist < max_dist &&...
                   ...
                   corner_table2_dist > d_table/2+0.4
                    ...
                    if 1 <= 8-nb_obstacle_cells && 8-nb_obstacle_cells <= 5 % valid possible basket location
            ...
            if corner_index(corner_num,1) ~= 0
                ...
                try
                    ...
                    if corner_num == 5
                        if not(explorationComplete)
                            fsm = 'exploration';
                        else % Need to continue to check unvisited corners even if there is no more unexplored cells
                            startChecking = true;
                        end
                    end
                    continue
...
%             figure(3)
            ...
            n_obst = sum(surrounding_cells);
            if not(n_obst >= 3 && n_obst <= 7)
                display(n_obst)
                stopTraject = true;
            end
            end
